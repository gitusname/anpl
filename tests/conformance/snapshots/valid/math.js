...
__anpl_modules["math"] = {
  ...
  main() {
    const __locals = Object.create(null);
    const __values = Object.create(null);
    let __block = "math.main.entry";
    while (true) {
      __anpl_check_runtime_limits();
      switch (__block) {
        case "math.main.entry": {
          ...
          __locals["math.main.result"] = __anpl_track_value(__values["%3"]);
          __values["%4"] = __anpl_track_value(__locals["math.main.result"]);
          return __values["%4"];
        }
      default:
        throw new Error(`Unknown ANPL MIR block ${__block}`);
