...
__anpl_modules["orders"] = {
  createOrder() {
    const __locals = Object.create(null);
    const __values = Object.create(null);
    let __block = "orders.createOrder.entry";
    while (true) {
      __anpl_check_runtime_limits();
      switch (__block) {
        case "orders.createOrder.entry": {
          __values["%1"] = __anpl_track_value("pending");
          __values["%2"] = __anpl_track_value({ "status": __values["%1"] });
          return __values["%2"];
        }
      default:
